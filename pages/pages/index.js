        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-500 mt-2">Track your loan repayments</p>
            </div>
            <form onSubmit={handleAuth} className="space-y-4">
              ...
              <div>
                ...
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg" placeholder="••••••••" required minLength={6} />
              </div>
            ...
            <p className="text-center mt-6 text-sm text-gray-600">
              ...
              <button onClick={() => { setIsSignUp(!isSignUp); setAuthError('') }} className="text-blue-600 font-medium hover:underline">{isSignUp ? 'Sign In' : 'Create Account'}</button>
            </p>
          </div>
        </div>
      </>
          ...
          <div className="max-w-6xl mx-auto px-4 py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowAddLoan(true)} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-sm font-medium">+ Add Loan</button>
                <button onClick={handleLogout} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg text-sm">Logout</button>
              </div>
            </div>
            <div className="flex gap-1 mt-4">
              {['overview', 'loans'].map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 rounded-lg text-sm font-medium ${activeTab === tab ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>{tab.charAt(0).toUpperCase() + tab.slice(1)}</button>
              ))}
            </div>
          </div>
        </header>
                    ...
                    {loans.filter(l => l.status === 'active').map(loan => (
                      <div key={loan.id} className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="font-medium text-gray-900 truncate">{loan.name}</h4>
                          <div className="w-full bg-gray-200 rounded-full h-2 mt-2"><div className="h-2 rounded-full" style={{ width: `${getProgress(loan)}%`, backgroundColor: getCategoryColor(loan.category) }} /></div>
                          <p className="text-xs text-gray-500 mt-1">{getProgress(loan).toFixed(0)}% paid</p>
                        </div>
            ...
            <div className="space-y-4">
              {loans.length === 0 ? (
                <div className="bg-white rounded-xl p-12 shadow-sm text-center">
                  ...
                  <button onClick={() => setShowAddLoan(true)} className="bg-blue-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-700">Add Loan</button>
                </div>
              ) : (
                loans.map(loan => (
                  <div key={loan.id} className="bg-white rounded-xl p-6 shadow-sm">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div><h3 className="font-semibold text-gray-900">{loan.name}</h3><p className="text-sm text-gray-500">{categories.find(c => c.value === loan.category)?.label} • Started {new Date(loan.start_date).toLocaleDateString()}</p></div>
                      </div>
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => handleDeleteLoan(loan.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg">🗑</button>
                      </div>
                    </div>
                    <div className="mb-4">
                      <div className="flex justify-between text-sm mb-2"><span className="text-gray-600">Progress</span><span className="font-medium">{getProgress(loan).toFixed(1)}%</span></div>
                      <div className="w-full bg-gray-100 rounded-full h-3"><div className="h-3 rounded-full transition-all" style={{ width: `${getProgress(loan)}%`, backgroundColor: getCategoryColor(loan.category) }} /></div>
                    </div>
                    ...
                    {loan.payments?.length > 0 && (
                      <div className="mt-4 pt-4 border-t"><p className="text-sm font-medium text-gray-700 mb-2">Recent Payments</p><div className="flex flex-wrap gap-2">{loan.payments.slice(-5).map(p => (<span key={p.id} className="px-3 py-1 bg-green-50 text-green-700 rounded-full text-sm">{formatMoney(p.amount)} • {new Date(p.payment_date).toLocaleDateString()}</span>))}</div></div>
                    )}
                  </div>
                ))
              )}
        ...
        {showAddLoan && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
              <div className="p-6 border-b flex justify-between items-center"><h2 className="text-xl font-semibold">Add New Loan</h2><button onClick={() => setShowAddLoan(false)} className="p-2 hover:bg-gray-100 rounded-lg">✕</button></div>
              <form onSubmit={handleAddLoan} className="p-6 space-y-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Loan Name *</label><input type="text" value={newLoan.name} onChange={(e) => setNewLoan({ ...newLoan, name: e.target.value })} placeholder="e.g., Car Loan from GTBank" className="w-full px-4 py-3 border rounded-lg" required /></div>
